...
tf.app.flags.DEFINE_string(
    ...
    'Model scope in the checkpoint. None if the same as the trained model.')
tf.app.flags.DEFINE_string(#checkpoint_exclude_scopes指定哪些层的参数不需要从vgg16模型里面加载进来  
    'checkpoint_exclude_scopes', None,
    'Comma-separated list of scopes of variables to exclude when restoring '
    'from a checkpoint.')
tf.app.flags.DEFINE_string(#通过trainable_scopes指定哪些层的参数是需要训练的，未指定的参数保持不变,若注释掉此命令，所有的参数均需要训练
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(deploy_config.inputs_device()):
            ...
            b_image, b_gclasses, b_glocalisations, b_gscores = r[0],r[1:7],r[7:13],r[13:19]
