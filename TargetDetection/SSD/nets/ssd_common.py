...
def tf_ssd_bboxes_encode_layer(labels,
                               ...
                               dtype=tf.float32):
    """训练过程中，首先需要将label信息（ground truth box，ground truth category）进行预处理，将其对应到相应的默认框上。
    根据默认框和ground truth box的IUO 重叠来寻找对应的默认框。文章中选取了jaccard重叠超过0.5的默认框为正样本，其它为负样本。
