...
class DataSet:
    ...
    def val_1w_loader(self):
        ...
        return generator_val_batch(data_list, dataset_path='train_1w', grid_attr=grid_attr, nbr_classes=nbr_classes,
                                   batch_size=100, img_height=img_height, img_width=img_width)
...
def generator_batch(data_list, dataset_path,grid_attr, nbr_classes=2, batch_size=1,
    ...
    for item in data_list:
        ...
        if  os.path.isfile(img_path):
            new_data_list_append('{},{}'.format(path_name,tag) )
            # continue
...
def generator_val_batch(data_list, dataset_path, grid_attr, nbr_classes=2, batch_size=1,
                        img_width=416, img_height=416):
    new_data_list = []
    new_data_list_append = new_data_list.append
    for item in data_list:
        path_name , tag = item.strip().split(',')
        img_path = os.path.join(os.getcwd(), "data", "car", "image", dataset_path, os.path.split(path_name)[-1])
        if  os.path.isfile(img_path):
            new_data_list_append('{},{}'.format(img_path,tag) )
            # continue
    data_list = new_data_list
    print(f"valid data have {len(data_list)} imgs ...")
