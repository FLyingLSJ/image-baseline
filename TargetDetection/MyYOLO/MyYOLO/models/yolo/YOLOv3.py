...
class YOLOv3(object):
    ...
    def calculate_object_confidence_loss(self, object_confidence_hat, object_confidence, object_mask, best_confidence_mask, scope='object_confidence_loss'):
        with tf.name_scope(scope):
            object_loss = tf.reduce_sum(object_mask * tf.square(object_confidence - object_confidence_hat) ) / self.batch_size
    ...
    def calculate_no_object_confidence_loss(self, no_object_confidence_hat, no_object_confidence, no_object_mask, ignore_mask, scope='no_object_confidence_loss'):
        with tf.name_scope(scope):
            no_object_loss = tf.reduce_sum(no_object_mask * tf.square(no_object_confidence - no_object_confidence_hat) ) / self.batch_size
    ...
    def calculate_wh_loss(self, label_object_mask, best_confidence_mask, twh_hat, twh, scope='wh_loss'):
        with tf.name_scope(scope):
            # twh = tf.sqrt(twh, name='twh');twh_hat = tf.sqrt(twh_hat, name='twh_hat')
    ...
    def total_loss(self, labels, scope='total_loss'):
        with tf.name_scope(scope):
            ...
            for i in range(len(self.scales)):
                ...
                self.label_object_mask_test = label_object_mask
                ...
                self.loss = tf.add_n([self.coord_alpha * (xy_loss + wh_loss),
