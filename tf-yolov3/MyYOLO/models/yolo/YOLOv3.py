...
class YOLOv3(object):
    ...
    def calculate_IOU(self, pred_box, label_box, scope='IOU'):
        """
        原始数据在处理的时候xy就是中心点的坐标，网络预测的是相对于中心点坐标的偏移量
        :param pred_box:
        :param label_box:
        :param scope:
        :return:
    ...
    def calculate_wh_loss(self, label_object_mask, best_confidence_mask, twh_hat, twh, scope='wh_loss'):
        with tf.name_scope(scope):
            # twh = tf.keras.backend.switch(twh > 0.0, twh, tf.zeros_like(twh))
            # twh_hat = tf.keras.backend.switch(twh_hat > 0.0, twh_hat, tf.zeros_like(twh_hat))
            # wh_loss = tf.reduce_sum(label_object_mask * best_confidence_mask * tf.square(tf.sqrt(twh) - tf.sqrt(twh_hat))) / self.batch_size
    ...
    def total_loss(self, labels, scope='total_loss'):
        with tf.name_scope(scope):
            ...
            for i in range(len(self.scales)):
                ...
                # 安装老师的代码best_confidence_mask的值全部为1.
    ...
    def get_box_and_score(self, scale, anchors):
        ...
        # 映射到原图在映射到输入的图像大小中0-1————》[416,416]————》[1069,500]
...
if __name__ == '__main__':    
    ...
    yolo = YOLOv3(darknet53, labels, 1)
