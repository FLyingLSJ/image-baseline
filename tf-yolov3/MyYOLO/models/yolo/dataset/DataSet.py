...
class DataSet:
    def __init__(self, data_dir, batch_size, training):
        ...
        if training:
            self.train_1w_filename_path = os.path.join('data', data_dir, 'train', 'train_final_1995.csv')
            self.train_b_filename_path = os.path.join('data', data_dir, 'train', 'train_b.csv')

            self.val_1w_filename_path = os.path.join('data', data_dir, 'val', 'train_final_1995.csv')
    ...
    def train_1w_loader(self):
        ...
        return  generator_batch(data_list, path='train_1w',grid_attr=grid_attr, nbr_classes=nbr_classes,
    ...
    def train_b_loader(self):
        ...
        return  generator_batch(data_list, path='train_b',grid_attr=grid_attr, nbr_classes=nbr_classes,
                                batch_size=batch_size, return_label=return_label, img_height=img_height,
                                img_width=img_width, shuffle=shuffle)
...
def generator_batch(data_list, path,grid_attr, nbr_classes=2, batch_size=1,
                    return_label=True, img_width=416, img_height=416, shuffle=True):
   
    # N = len(data_list)
    # print("datalist:{}".format(N))
    new_data_list = []
    new_data_list_append = new_data_list.append
    for item in data_list:
        path_name , tag = item.strip().split(',')
        img_path = os.path.join(os.getcwd(), "data", "car", "image", path, path_name)
    ...
    while True:
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            image_path = os.path.join(os.getcwd(), "data", "car", "image", path, img_path)
            ...
            for scale_size in cfg.SCALES:
                ...
                # position_list，图像中目标车辆的坐标和标签，非车辆为0
                for position in position_list:
                    if position[0] == '' or len(position) != 5:
                        continue
                    
                    # 此时w，h是在resize之后的图像上，xy是在卷积的特征图上
                    w = float(position[2]) * cfg.X_SCALE  # 原始图像和进入网络时的压缩比例 416/1069
                    h = float(position[3]) * cfg.Y_SCALE
                    x = (float(position[0]) * cfg.X_SCALE + w / 2) / (416 / scale_size) # x,y在特征图上的坐标
                    y = (float(position[1]) * cfg.Y_SCALE + h / 2) / (416 / scale_size) # 图像显示时左上角的坐标为0,0
                    c = float(position[4])

                    grid_x = int(x) # 范围在0~12
                    grid_y = int(y)

                    # maxtric是一个高度稀疏的矩阵，只含有目标车辆的信息，xywh，[13,13,6]
